...
# Starts MIX server instances on each machine specified
...
# Launches the MIX servers in remote hosts
for slave in `echo "$HOSTLIST" | sed  "s/#.*$//;/^$/d"`; do
  ssh $HIVEMALL_SSH_OPTS "$slave" "$HIVEMALL_HOME/sbin/mixserv-daemon.sh" start 2>&1 | sed "s/^/$slave: /" &
done
